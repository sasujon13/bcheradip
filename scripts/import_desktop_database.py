...
def main():
    ...
    files = [
        "cheradip_country.sql", "cheradip_location.sql", "cheradip_banbeis.sql", "cheradip_institutes.sql",
        "cheradip_merit5.sql", "cheradip_merit6.sql", "cheradip_merit7.sql",
        "cheradip_recommend5.sql", "cheradip_recommend6.sql",
        "cheradip_vacancy5.sql", "cheradip_vacancy6.sql", "cheradip_vacancy7.sql",
        "cheradip_token.sql",
    ]
    for f in files:
        path = SOURCE_DIR / f
        ...
        if code == 0:
            ...
            if err.strip():
                print(err.strip()[:500])
    copies = [
        ("cheradip_token -> cheradip_tokens",
         "INSERT IGNORE INTO cheradip_tokens (id, Token, Counter, Status, purpose, expires_at, created_at, updated_at) "

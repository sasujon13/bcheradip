...
for %%F in ("%DIR%\cheradip_country.sql" "%DIR%\cheradip_location.sql" "%DIR%\cheradip_banbeis.sql" "%DIR%\cheradip_institutes.sql" "%DIR%\cheradip_merit5.sql" "%DIR%\cheradip_merit6.sql" "%DIR%\cheradip_merit7.sql" "%DIR%\cheradip_recommend5.sql" "%DIR%\cheradip_recommend6.sql" "%DIR%\cheradip_vacancy5.sql" "%DIR%\cheradip_vacancy6.sql" "%DIR%\cheradip_vacancy7.sql" "%DIR%\cheradip_token.sql") do (
  if exist %%F (
    echo Importing %%F ...
...
"%MYSQL%" -u %USER% --default-character-set=%CHARSET% %DB% -e "INSERT IGNORE INTO cheradip_tokens (id, Token, Counter, Status, purpose, expires_at, created_at, updated_at) SELECT id, CAST(Token AS UNSIGNED), COALESCE(CAST(Counter AS CHAR), ''), Status, NULL, NULL, NOW(), NOW() FROM cheradip_token;"
